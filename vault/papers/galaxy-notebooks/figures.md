...
**Figure count:** the paper has **4 referenced figures** (Fig 1–4), all backed by captured panels and embedded in the manuscript. One optional figure (PDF-renderer) has assets ready but is not yet referenced. Final realistic count: 4, or 5 with the PDF-renderer figure.
...
Multi-panel: (a) differential ATAC-seq before/after — off-graph notebook seeding 2 inputs / 0 tools vs on-graph seeding the 5-step DAG; (b) differential-ChIP caller/comparator split with a "analyst picks the two conditions" seam. Table 1 accompanies.
- Asset status: **COMPLETE** (panels). (a) `uc3_fig3a_workflow_degenerate.png` + `uc3_fig3a_workflow_5step.png` (same zoom, read as a pair). (b) `uc2_fig3b_caller_5step.png` + `uc2_fig3b_comparator_29step.png`; optional reference `uc2_single_condition-pinned_34step.png`. Remaining: panel layout + the dashed seam arrow in final art. The degenerate "before" is a real reproduced artifact (workflow `4b187121143038ff`), extracted from the off-graph page's two re-uploaded-PNG references — not a mock-up.
...
- **PDF renderer extension (assets READY).** Demonstrates the Implementation "PDF tool output → display + extract" contribution, which currently has no figure. Pairs the live `<embed>` view (PDF chrome) against the rasterized `history_dataset_as_pdf` directive, single-page (volcano) vs multi-page page-selection (PCA, page 1 of 5); the PCA embed shot visibly shows the known multi-page "peek" residual. Assets: `uc3_volcano_embed.png`, `uc3_volcano_as_pdf_directive.png`, `uc3_pca_embed.png`, `uc3_pca_as_pdf_directive.png`. Slots beside the Implementation PDF-renderer paragraph.
...
## Source PNGs
15 PNGs in `figures/` (source object IDs in `figures/MANIFEST.md`). The manuscript embeds them with note-relative paths (`![](figures/…png)`) — single source of truth, no `public/` mirror. Astro's content-collection image pipeline hashes/optimizes them into the build, and the same relative embeds render in Obsidian.
</content>
