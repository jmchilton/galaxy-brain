...
**Table 1. Notebook-driven extraction across three vignettes.** Step counts include the input node(s) and all recovered tool steps. "Map-over steps" counts tool steps that ran as collection map-overs. "Exposed outputs" counts workflow outputs surfaced by the extraction. "Dangling" counts unresolved input connections in the extracted workflow. "Report warnings" counts leftover dataset-instance identifiers requiring manual repair in the seeded report. "Science-identical" indicates whether re-running the extracted workflow reproduced the original results. Vignettes 2 and 3 each contribute two rows — a boundary case and its resolution. The Vignette 3 off-graph row is a deliberate negative control: the *same* tool-driven analysis documented with re-uploaded screenshots instead of on-graph outputs, which seeds nothing and so isolates display — not computation — as what gates extraction. An em-dash (—) marks a metric that does not apply to a given row.
...
| Vignette 3 — differential ATAC-seq (off-graph, negative control) | 2 (0 tools) | 0 | 0 | — | — | — (no workflow seeded) |
| Vignette 3 — differential ATAC-seq (on-graph, same analysis) | 5 | — | 3 | 0 | — | reproduced |
...
Extracted as a single workflow, the analysis comes out complete but condition-pinned: thirty-four steps, zero dangling inputs, three outputs, with the comparison seam pinned to the specific condition identifiers (`identifier=G1E` / `mega`). This single workflow re-executes and reproduces the analysis faithfully; it is simply tied to the two conditions it was documented on, so reuse on new conditions means editing the seam. Galaxy turns that boundary into reuse by extracting two workflows: a map-over **peak caller** (read collections in, per-condition peak sets out: five steps) and a pairwise **comparator** (any two peak sets in, differential result out: twenty-nine steps, three outputs). Each is independently reusable; only their composition — which two conditions to contrast — remains analyst-supplied. The split is the engineering answer the notebook surface makes possible: it renders the seam visible, then decomposes the condition-pinned artifact into reusable parts.
