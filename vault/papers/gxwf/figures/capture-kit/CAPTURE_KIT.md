...
- **Fig S6 — conditional-aware completion.** In `krona-demo.gxwf.yml`, use **step 4** `taxonomy_krona_chart` `type_of_data.type_of_data_selector` (cases `text` / `taxonomy`). Set the selector to `text`, trigger completion inside `type_of_data:` → offers only `input`; set it to `taxonomy` → offers `input` **and `max_rank`**. The appearance of `max_rank` only in the taxonomy case is the case-aware-completion proof. **Do not** use `kraken2`'s `single_paired` conditional: every case exposes the same `input_sequences` key (only data arity differs), so the offered keys don't change. Captured: `web-shots/conditional-completion-{text,taxonomy}.png`.
