Targets `release_26.1` — this is a bug fix, not new behaviour.
...
Both date to `1f95f421c92` ("Tool markdown reports") and are unchanged on
`release_26.1` and `dev` alike.
...
Two API regression tests in `lib/galaxy_test/api/test_datasets.py` upload a
`tool_markdown` dataset with a bad label and assert 400 plus the label in
`err_msg`. This reproduces #21198 without a custom tool, since the endpoint
resolves against the *creating* job and an upload job has none of these
labels.

Verified red first on `release_26.1` — both failed with `Request status code
(500) was not expected value 400` — then green after the fix.

| Check | Result |
|---|---|
| `test_datasets.py -k report_for_tool_markdown` | 2 passed (red 2 failed first) |
| `test/unit/app/managers/` | 395 passed |
